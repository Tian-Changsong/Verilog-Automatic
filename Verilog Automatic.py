import sublime, sublime_plugin,re,os,ctagsplugin,time
	...
	def insert_list(self,edit,name_list_to_insert,bitwidth_list_to_insert,insert_point):
		...
		for i in range_list:
			if bitwidth_list_to_insert[i] is not None:
				self.view.insert(edit,insert_point,'.'+name_list_to_insert[i]+'('+name_list_to_insert[i]+bitwidth_list_to_insert[i]+')')
			else:
